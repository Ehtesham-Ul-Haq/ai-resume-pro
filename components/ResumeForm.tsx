...
import { User, Briefcase, Award, Code, CheckCircle2, AlertCircle, TerminalSquare } from 'lucide-react';
  ...
  return (
    <div>
      ...
      >
        ...
        <SectionContainer title="Personal Data Input" icon={<User className="w-6 h-6 text-blue-400" />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            />
             <div className="md:col-span-2">
                <TextareaField
                    placeholder="Provide a concise, impactful summary to highlight your professional identity and aspirations."
        ...
        <SectionContainer title="Professional Chronicle & Academia" icon={<Briefcase className="w-6 h-6 text-purple-400" />}>
          <TextareaField
            placeholder="Detail your work history with responsibilities, achievements, and quantified results."
          ...
          />
          <TextareaField
            placeholder="Enter academic degrees, certifications, institutions, and completion dates."
        ...
        <SectionContainer title="Competencies & Innovations" icon={<Award className="w-6 h-6 text-green-400" />}>
          <InputField
            type="text"
            placeholder="e.g., Python, SQL, Machine Learning, Cloud Computing (AWS/Azure)"
            label="Technical Skill Set (Comma-separated)"
            {...register("technicalSkills")}
          />
          <InputField
            type="text"
            placeholder="e.g., Problem-solving, Critical Thinking, Adaptability, Collaboration"
            label="Soft Skill Set (Comma-separated)"
            {...register("softSkills")}
            className="mt-6"
          />
          <InputField
            type="text"
            placeholder="e.g., English (Native), Spanish (Fluent), German (Intermediate)"
            label="Language Proficiency (Comma-separated)"
            {...register("languages")}
            className="mt-6"
          />
          <TextareaField
            placeholder="Showcase key projects, detailing your role, methods, and positive outcomes."
            label="Project Portfolio (Optional)"
            {...register("projects")}
            rows={5}
            className="mt-6"
          />
...
const InputField = forwardRef(
  ({ type, placeholder, label, error, errorMessage, className, ...rest }: any, ref: any) => (
    <div className={`w-full ${className || ''}`}>
      ...
      />
      {error && <p className="text-red-400 text-sm mt-1 ml-2">{errorMessage}</p>}
    </div>
  )
);
...
const TextareaField = forwardRef(
  ({ placeholder, label, error, errorMessage, rows, className, ...rest }: any, ref: any) => (
    <div className={`w-full ${className || ''}`}>
      <label className="block text-gray-300 text-sm font-semibold mb-1">
        {label} {error && <span className="text-red-400">*</span>}
      </label>
